...
export default function PostCaption({action = () => {}}: {action?: any}) {
  ...
  return (
    ...
    <div className='w-[300px] h-[300px] px-2'>
      <textarea name="caption" id="post-caption" cols={20} rows={10} className='w-full resize-none bg-transparent outline-none border-none text-[.9rem]' placeholder='Write a caption...' minLength={0} maxLength={maxCaptionLen} ref={captionRef} onChange={handleInput}></textarea>
      <div className='flex items-center justify-between'>
        <span className='icon only-icon text-[1.5rem]' onClick={chooseEmoji}>
          <MdOutlineEmojiEmotions />
        </span>
        <p className="caption-word-count text-[.8rem] text-secondary-color">{captionLen}/{maxCaptionLen}</p>
      </div>
